...
pg.setConfigOption('background', colors['lightest'])
pg.setConfigOption('foreground', colors['dark'])
...
def series_plot(plt, y_vals, idx=100, r=0, xall=False):
    ...
    line.setPen(color=colors['darker'], alpha = 0.5, width=0)

    scat = pg.ScatterPlotItem(x=t, y=y, size=s, name='series')
    scat.setPen(color=(1,1,1,0), width=1.0)
    scat.setBrush(color=colors['darker'])
    plt.addItem(scat)
...
def bifurc_plot(plt, y_vals, r=0, ipop=0.5, discard=64):
    ...
    if len(plt.items) == 0:
        ...
        bifurcation.setBrush(color=colors['darker'], alpha=0.5, width=1)
        bifurcation.setPen(color=(1,1,1,0), width=0, alpha=0.5)
        plt.addItem(bifurcation)

        plt.titleLabel.item.setFont(txtfont)

        xaxis = plt.getAxis("bottom")
        #xaxis.setTickSpacing(0.1, 0.1)
        ...
        bs = [brushes['darker'] for b in range(len(xs))]
...
class Controls(QWidget):
    def __init__(self, variable='', parent=None):
        ...
        self.cobweb_box = QtWidgets.QCheckBox('Web plot', parent=self)
        ...
        self.ipop_box.setSingleStep(0.005)
        self.ipop_box.setDecimals(5)
        ...
        self.precision_box1.setChecked(1)

        self.precision_box2 = QtWidgets.QRadioButton("0.005", parent=self)
        self.l6.addWidget(self.precision_box2)
        self.precision_box2.setChecked(0)

        self.precision_box3 = QtWidgets.QRadioButton("0.001", parent=self)
        self.l6.addWidget(self.precision_box3)
        self.precision_box3.setChecked(0)


        self.verticalLayout.addLayout(self.l6)
        ...
        self.precision_box1.setStyleSheet(QCheckBox_style)
        self.precision_box2.setStyleSheet(QCheckBox_style)
        self.precision_box3.setStyleSheet(QCheckBox_style)
    ...
    def setValues(self, kind=''):
        ...
        if kind == 'rate_slider':
            ...
            # defaults
            self.animrate.setValue(100)
            self.rate_box.setValue(0.0)
            self.ipop_box.setValue(0.5)
...
class Widget(QWidget):
    def __init__(self, app, parent=None):
        ...
        self.setStyleSheet(f"Widget {{ background-color: {colors['lightest']}; }}")
        ...
        self.setWindowTitle("Logistic map visualization")
    ...
    def animate_plot(self):
        ...
        if self.controls.rateval <= 3.998:
            if self.controls.precision_box1.isChecked():
                self.controls.rateval += 0.01
            elif self.controls.precision_box2.isChecked():
                self.controls.rateval += 0.005
            elif self.controls.precision_box3.isChecked():
                self.controls.rateval += 0.001
        else: return
        #self.rate_box.setValue(self.controls.rateval)

        #print(self.controls.rateval)
